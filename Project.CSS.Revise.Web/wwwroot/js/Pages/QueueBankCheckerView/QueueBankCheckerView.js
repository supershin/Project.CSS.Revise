...
function initCounterModeButtons() {
    ...
    // 🟡 โหมด QR → เรียก /QueueBankCheckerView/CounterQr ต่อ counter
    function setQRMode() {
        const rootPath = (typeof baseUrl !== "undefined" ? baseUrl : "/");
...
async function onCounterBadgeClicked(e) {
    ...
    // ✅ Confirm message (use your common function)
    const msg = (type === "bank")
        ? "Remove BANK from this counter?"
        : "Remove UNIT from this counter?";

    const confirmed = await confirmMessage(msg, {
    ...
    try {
        ...
        if (type === "bank") {
            if (!registerLogId || !bankId) {
                errorToast("Bank or register is invalid.");
                return;
            }

            // call API (your existing function)
            const ok = await callCheckoutBankCounter({
            ...
            if (ok !== false) {
                successToastV2("Bank removed");
                await afterRemoveBadge(counterNo);
            } else {
                errorToast("Remove bank failed");
            }
        ...
        // ---------- UNIT ----------
        if (!projectId || !unitId) {
            errorToast("Project or Unit is invalid.");
            return;
        }

        const ok = await callRemoveUnitRegister({
            ProjectID: projectId,
            UnitID: unitId,
        ...
        if (ok !== false) {
            successToastV2("Unit removed");
            await afterRemoveBadge(counterNo);
        } else {
            errorToast("Remove unit failed");
        }

    } catch (err) {
        console.error("❌ remove badge error:", err);
        errorToast("System error");
    } finally {
        hideLoading();
    }
}

async function afterRemoveBadge(counterNo) {
    // ✅ วิธี 1: รีโหลด detail ใหม่ (แนะนำ)
    if (counterNo) {
        await loadCounterDetail(counterNo);
        return;
    }
}
